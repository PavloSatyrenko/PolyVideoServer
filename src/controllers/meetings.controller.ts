...
export const meetingsController = {
    async createMeeting(request: Request, response: Response): Promise<void> {
        try {
            ...
            const isRecurring: boolean | undefined = request.body.isRecurring;
            const ownerId: string = request.user!.id;

            const meeting: Meeting = await meetingsService.createMeeting(ownerId, title, isPlanned, startTime, endTime, isRecurring);
